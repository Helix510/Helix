...
export default function NewsFeed({ ticker, companyName }) {
  ...
  return (
    <div className="bg-surface p-8 rounded-3xl border border-border shadow-none mb-6 font-['DM_Sans']">
      <h3 className="text-lg font-bold text-primary mb-8 tracking-tight">Latest News</h3>
        ...
        {news.length === 0 ? (
          ...
          news.map((item, i) => (
            <a 
              ...
              className="group relative block pl-6 hover:bg-input/50 transition-all rounded-xl -ml-2 p-2"
            >
              <div className="absolute left-2 top-2 bottom-2 w-1 rounded-full bg-gray-600 group-hover:bg-primary transition-colors"></div>
              <h4 className="text-sm font-bold text-primary group-hover:text-primary/80 transition-colors mb-1">{item.title}</h4>
              <div className="flex items-center gap-3 text-[10px] font-bold text-muted uppercase tracking-widest mb-1">
